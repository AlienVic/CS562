...
public class GenerateFile {
	...
	public GenerateFile(String fileName, String packageName,
	...
	public void printMFStruture(ConnectDB connectdb){
		returnValue.append("\r\n class MFStruture{\r\n");
		ArrayList<String> attrubutes = new ArrayList<String>();
		
		
		// handle V part
		ArrayList<String> list  = varibles.getV();
		...
		for (int j = 0; j < S.size(); j++) {
			...
			if(!(attrubutes.contains(name))&&!(name.contains("_"))){
				String type = connectdb.getTableStructure().get(name);
				attrubutes.add(name);
				returnValue.append("\tprivate " + type + " " + name + ";\r\n");
				String 	upperCaseString =  name.substring(0, 1).toUpperCase() + name.substring(1);
				//set function
	...
	public void printClass(ConnectDB connectdb) {
	...
	public void printDatabaseConnt(ConnectDB connectdb){
		
		StringBuffer database = new StringBuffer();
		Properties prop =  ConnectDB.getProp();
	...
	public void printFirstScan(ConnectDB connectdb){
		// we initial a variable and append it at last
		StringBuffer firstScan = new StringBuffer();

		String tableNameString = connectdb.getTableName();
		...
		for (int j = 0; j < tupleV.size(); j++) {
			...
			if(connectdb.getTableStructure().get(tupleV.get(j)).equals("int")){
		...
		for (int i = 0; i < this.mfvaribles.size(); i++) {
			...
			}else if(connectdb.getTableStructure().containsKey(mfvariable)){
	...
	private void printHandleMFstructure(ConnectDB connectdb) {
		...
		for (int i = 0; i < number; i++) {
			ArrayList<String> sigma = sigmas.get(i);
			...
			//create a temporary variable t for temporary store the information for each tuple
			for(Map.Entry<String, String> entry:tableName.entrySet()){
				String upperCaseString =  entry.getKey().substring(0, 1).toUpperCase() + entry.getKey().substring(1);
				String upperCaseString1 =  entry.getValue().substring(0, 1).toUpperCase() + entry.getValue().substring(1);
			...
			for (int j = 0; j < sigma.size(); j++) {
				...
				String right = "";
				String operater = "";
				HashMap<String, String> tableStructure = connectdb.getTableStructure();
				if(sg.contains("=")){
					operater = "=";
				}else if(sg.contains("<>")){
					operater = "<>";
	...
	private void printHavingClause() {
	...
	private void printFinalResult(ConnectDB connectdb) {
		...
		for (int i = 0; i < select.size(); i++) {
			...
			length = attr.length();
			if(connectdb.getTableStructure().containsKey(attr)){
				length = 8;
			}
			if(attr.contains("_")){
			...
			}else if(connectdb.getTableStructure().containsKey(attr)){
				if(connectdb.getTableStructure().get(attr).equals("int")){
					finalString.append("\tSystem.out.printf(\"%-"+length+"s\",\""+attr+"\");\r\n");
				}else{
					finalString.append("\tSystem.out.printf(\"%-"+length+"s\",\""+attr+"\");\r\n");
				}
	...
	public String write(ConnectDB connectdb) {
